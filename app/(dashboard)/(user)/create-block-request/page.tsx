...
//       } else if (type === 'number') {
//         (newData as any)[key] = parseFloat(value) || 0;
//       } else {
...
//       // Show the review modal instead of submitting directly
//       setShowReviewModal(true);
//     } catch (error) {
//       console.error("Form validation error:", error);
//       setFormError("An error occurred while validating the form. Please try again.");
//     } finally {
//       setFormSubmitting(false);
//     }
//   };
...
//         processedLineSections: processedLineSections.map(section => ({
...
//         setCustomActivity("");
//         setErrors({});
//         // Redirect to block request list
//         router.push("/block-request/list");
//       }
//     } catch (error: any) {
...
//         <span className="text-4xl font-extrabold text-black text-center" style={{ letterSpacing: '0.02em' }}>Enter New Block Request</span>
//       </div>
//       {/* Blue rounded box for Date and Major Section */}
//       <form onSubmit={handleSubmit} className="w-full flex justify-center mt-0">
//         <div className="rounded-t-3xl rounded-b-2xl bg-[#c6e6f7] border-2 border-black p-6 pt-4 w-[600px] flex flex-col items-start" style={{ boxShadow: '0 4px 12px #0002', borderTopLeftRadius: '32px', borderTopRightRadius: '32px', borderBottomLeftRadius: '24px', borderBottomRightRadius: '24px' }}>
...
//                     onChange={(selected) => {
//                       const values = selected ? selected.map((opt: any) => opt.value).join(',') : '';
//                       if (isYard) {
//                         handleRoadSelection(block, values);
//                       } else {
...
//                 {/* <span className="border-2 border-[#b71c1c] bg-[#fffbe9] px-1">IF YES</span> */}
...
//                 <input name="freshCautionSpeed" value={formData.freshCautionSpeed || ''} onChange={handleInputChange} placeholder="Speed" className="border-2 border-[#b71c1c] bg-[#fffbe9] text-black placeholder-black px-1 w-12 text-[13px]" />
...
export default function CreateBlockRequestPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Prepare the data for submission (same as in handleConfirmedSubmit)
      const submitData: UserRequestInput = {
        ...formData,
